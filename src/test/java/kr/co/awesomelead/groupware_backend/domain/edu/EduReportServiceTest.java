...
public class EduReportServiceTest {
    ...
    void createEduReport_Success() throws IOException {
        ...
        EduAttachment eduAttachment = EduAttachment.builder()
            ...
            .s3Key("https://s3.amazonaws.com/bucket/attachment.pdf")
            .build();
        ...
        //when
        eduReportService.createEduReport(requestDto,
    ...
    void getEduReports_Success() {
        ...
        when(userRepository.findById(1L)).thenReturn(Optional.of(user));
        when(eduReportRepository.findEduReportsWithFilters(EduType.SAFETY, user.getDepartment(),
            user))
            .thenReturn(mockList);
    ...
    void markAttendance_Success() throws IOException {
        ...
        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
        when(
            eduAttendanceRepository.existsByEduReportAndUser(any(EduReport.class), any(User.class)))
            .thenReturn(false);
        ...
        verify(eduAttendanceRepository, times(1))
            .save(any());
    }
