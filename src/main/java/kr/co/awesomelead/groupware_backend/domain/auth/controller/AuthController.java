...
public class AuthController {
    ...
    public ResponseEntity<ApiResponse<Void>> checkAccountByEmail(
            @Valid @RequestBody VerifyAccountByEmailRequestDto requestDto) {
        authService.checkAccountByEmail(requestDto.getEmail());
    ...
    @Operation(
            ...
            description =
                    "이메일 계정 확인(`POST /api/auth/check-account/email`)과 이메일 인증을 완료한 후, 비밀번호를"
                            + " 재설정합니다.")
    @ApiResponses(
            value = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
    ...
    @Operation(
            ...
            description =
                    "이메일/전화번호 계정 확인(`POST /api/auth/check-account/phone`)과 휴대폰 인증을 완료한 후, 비밀번호를"
                            + " 재설정합니다.")
    @ApiResponses(
            value = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
