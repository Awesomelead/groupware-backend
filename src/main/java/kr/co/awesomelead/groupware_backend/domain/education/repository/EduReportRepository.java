...
public interface EduReportRepository extends JpaRepository<EduReport, Long> {
    ...
    @Query(
            "SELECT new"
                + " kr.co.awesomelead.groupware_backend.domain.education.dto.response.EduReportSummaryDto(r.id,"
                + " r.title, r.eduType, r.eduDate, COALESCE((SELECT ea.attendance FROM"
                + " EduAttendance ea WHERE ea.eduReport = r AND ea.user = :user), false), r.pinned)"
                + " FROM EduReport r WHERE (:type IS NULL OR r.eduType = :type) AND (r.eduType !="
                + " 'DEPARTMENT' OR r.department = :dept) ORDER BY r.pinned DESC, r.eduDate DESC")
    List<EduReportSummaryDto> findEduReportsWithFilters(
            @Param("type") EduType type, @Param("dept") Department dept, @Param("user") User user);
}
