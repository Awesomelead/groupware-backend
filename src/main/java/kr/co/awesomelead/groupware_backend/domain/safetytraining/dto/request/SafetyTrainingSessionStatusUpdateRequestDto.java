...
@Getter
@NoArgsConstructor
@Schema(description = "안전보건 교육일지 상태 변경 요청")
public class SafetyTrainingSessionStatusUpdateRequestDto {

    @NotNull(message = "교육일지 상태는 필수입니다.")
    @Schema(
            description = "교육일지 상태 코드 (OPEN: 진행중, CLOSED: 정상 마감)",
            example = "CLOSED",
    ...
    @Schema(
            description =
                    "교육 미참석 사유(교육일지 단위). status=CLOSED + 결석자 존재 시 필수, "
