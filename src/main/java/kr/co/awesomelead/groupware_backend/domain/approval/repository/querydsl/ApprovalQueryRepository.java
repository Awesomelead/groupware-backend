...
public class ApprovalQueryRepository {
    ...
    public Page<ApprovalSummaryResponseDto> findApprovalsByCondition(
        ...
        List<ApprovalSummaryResponseDto> dtoList = approvals.stream()
     ...
     */
    private BooleanExpression getCategoryExpression(ApprovalListRequestDto condition, Long userId,
        String userRole) {
        ...
        return switch (category) {
            ...
            case REFERENCE -> getReferenceCategoryExpression(condition.getStatus(),
                condition.getParticipantType(), userId);
    ...
    private BooleanExpression getAllCategoryExpression(Long userId, String userRole) {
        ...
        return approval.drafter.id.eq(userId) // 내 작성
    ...
    private BooleanExpression getInProgressCategoryExpression(ApprovalStatus status, Long userId) {
        ...
        return switch (status) {
            case WAITING -> baseCondition.and(
                approval.steps.any().approver.id.eq(userId)
                    .and(approval.steps.any().status.eq(ApprovalStatus.PENDING))); // 내 차례
            case APPROVED -> baseCondition.and(
                approval.steps.any().approver.id.eq(userId)
                    .and(approval.steps.any().status.eq(ApprovalStatus.APPROVED))); // 내가 기결함
            case REJECTED -> baseCondition.and(
                ...
                approval.steps.any().approver.id.eq(userId)
                    ...
                    .or(approval.status.in(ApprovalStatus.REJECTED, ApprovalStatus.CANCELED)));
     ...
     */
    private BooleanExpression getReferenceCategoryExpression(ApprovalStatus status,
        ParticipantType participantType,
        Long userId) {
        // 1. 참조자(REFERRER)인 경우: 상신 직후부터 전체 노출
        BooleanExpression isReferrer = approval.participants.any().user.id.eq(userId)
            .and(approval.participants.any().participantType.eq(ParticipantType.REFERRER));

        // 2. 열람권자(VIEWER)인 경우: 최종 승인(APPROVED)된 문서만 노출
        BooleanExpression isViewerAndApproved = approval.participants.any().user.id.eq(userId)
